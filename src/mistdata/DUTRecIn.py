...
class DUTRecIn:
    ...
    def write_self_to_file(self, file: h5py.File):
        ...
        grp.create_dataset(
            "s11_antenna_time", data=dtlist2strlist(self.s11_antenna_time)
        )
        grp.create_dataset(
            "s11_ambient_time", data=dtlist2strlist(self.s11_ambient_time)
        )
        grp.create_dataset(
            "s11_noise_source_time",
            data=dtlist2strlist(self.s11_noise_source_time),
        )
    ...
    def read_self_from_file(cls, file: h5py.File):
        """
        :param file: The h5py.File object representing the HDF5 file
        containing the DUTRecIn data.
        :return: An instance of the DUTRecIn class with data from the file.
        """
        obj = cls()
        grp = file["dutrecin"]
        ...
        obj.s11_noise_source_time = hdfdt2dtlist(
            grp.get("s11_noise_source_time")
        )
    ...
    def cut_freq(self, freq_min=None, freq_max=None):
        """
        Cut the frequency range of the DUTRecIn object.

        :param freq_min: The minimum frequency to keep in MHz.
        :param freq_max: The maximum frequency to keep in MHz.
        :return: None
        """
        if freq_min is None:
        ...
        self.s11_antenna = self.s11_antenna[..., mask]
        self.s11_ambient = self.s11_ambient[..., mask]
        self.s11_noise_source = self.s11_noise_source[..., mask]
    ...
    def __add__(self, other):
        ...
        if np.any(self.s11_freq != other.s11_freq):
            raise ValueError("Frequency data does not match")
    ...
    def __eq__(self, other):
        ...
        return (
            ...
            and np.all(self.s11_antenna_time == other.s11_antenna_time)
            and np.all(self.s11_ambient_time == other.s11_ambient_time)
            and np.all(
                self.s11_noise_source_time == other.s11_noise_source_time
            )
        )
